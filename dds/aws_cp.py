...
def _inventory_load_from_db_file():
    with open(PATH_AWS_CP_DB) as f:
        return json.loads(f.read())


def _inventory_save_to_db_file(db: dict):
    with open(PATH_AWS_CP_DB, 'w') as f:
        f.write(json.dumps(db, indent=4))


def aws_cp_build_inventory_of_dl_files_folder():
...
def aws_cp_init():
    ...
    db = aws_cp_build_inventory_of_dl_files_folder()
    _inventory_save_to_db_file(db)


# function called after a file is successfully AWS copied
def aws_cp_add_to_db_file(file_name, file_size):
    try:
...
def aws_cp_compare_built_inventory_vs_db_file(d_new):
    d_diff = {}
    d_old = _inventory_load_from_db_file()
