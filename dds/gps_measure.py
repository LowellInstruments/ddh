...
# pu_gps: port USB gps
g_pu_gps = gps_get_quectel_usb_port_from_file('gps')
g_pu_ctl = gps_get_quectel_usb_port_from_file('ctl')
if not g_pu_gps and not g_pu_ctl:
    if not is_this_telit_cell():
        g_pu_gps, g_pu_ctl = detect_quectel_usb_ports()
# fallback
if not g_pu_gps:
    g_pu_gps = '/dev/ttyUSB1'
if not g_pu_ctl:
    g_pu_ctl = '/dev/ttyUSB2'

# this is nice, it even fallbacks if config is for
# external GPS puck, but it is not present
_g_bu353s4_port = gps_utils_bu353s4_find_usb_port() if \
...
def _open_gps():
    ...
    if _g_bu353s4_port:
        ...
        _g_sp = serial.Serial(g_pu_gps, baudrate=115200, timeout=0.2,
...
def _gps_read():
    ...
    while time.perf_counter() < till:
        ...
        if b:
            break
    _close_gps()
    # with bad signal, GPS frames come out empty, full of ','
    return b
